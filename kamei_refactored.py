        ...
        for commits in commit_list:
            for key in commits.stats.files:
                if file == key:
                    ...
                    commits_list.append({'committed_datetime': commits.committed_datetime.date(),'hexsha': commits.hexsha})
    ...
    for i in range(len(commit_list)):
        ...
        for this_commit_dict in file_touched_commit_list:
            if this_commit_dict['file'] in this_files:
                age =age + (commit.committed_datetime.date()-this_commit_dict['commit'][-1]['committed_datetime']).days
    ...
    for i in range(commit_list.size):
        ...
        for file_info in file_touched_dict_list:
            if len(file_info['file'].split('/')) >= 2:
                ...
                sub_dict_list=np.append(sub_dict_list,{'sub_sys': file_info['file'].split('/')[0],'subsys_devs': subsystem_dev_list})
        ...
        for subsysname in sub_sys_name_set:
            ...
            aggrigated_sub_dict_list = np.append(aggrigated_sub_dict_list, {'sub_sys': subsysname, 'devs': aggrigated_dev_list})
...
def diffusion_metrics_extraction(commit_list):
    ...
    for commit in commit_list:
        ...
        diffusion_list_to_return.append({'commit': commit.hexsha, 'ns': ns, 'nd': nd, 'nf': nf, 'entropy': entropy, 'purpose': purpose})
...
def main():
    ...
    commit_list = np.array(commits_list)
